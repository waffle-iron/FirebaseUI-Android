...
package object auth {
  ...
  def getUserInfo: Option[UserInfo] = {
    ...
    Await.result({if (haveGooglePlayServices) Future.successful{
      val user = FirebaseAuth.getInstance().getCurrentUser
      //      Log.d("firebase", user.toString)
      (Try(UserInfo(
        uid = user.getUid, displayName = user.getDisplayName,
        photoURL = Try{user.getPhotoUrl.getPath}.getOrElse(""),
    ...
    } else AuthOnJavascript.getAuthOnJavascript.getUserInfo} recover{case ex =>Log.e("UserInfo",ex.toString);None},5.second)
  }
